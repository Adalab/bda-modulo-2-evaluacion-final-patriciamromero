...
FROM film 
WHERE rating LIKE "PG-13"; -- the title displayed will be filtered under this condition 
...
SELECT rating AS Clasificacion, COUNT(rating) AS Recuento -- the columns have been renamed in order to show the info clearly
FROM film
...
SELECT title AS Titulo
FROM film
WHERE rating = "R" AND length > 120; -- using floor allows us to transform film lenght into hours and compare it to the amount indicated
